...
public class TopRightMenu {
    ...
    public TopRightMenu showAsDropDownM(View anchor,int diff){
        if (mPopupWindow == null){
            getPopupWindow();
        }
        mPopupWindow.getContentView().measure(View.MeasureSpec.UNSPECIFIED,View.MeasureSpec.UNSPECIFIED);
        int i = mPopupWindow.getContentView().getMeasuredWidth();
           i=i-anchor.getWidth()/2-diff;
        if (!mPopupWindow.isShowing()) {
            mPopupWindow.showAsDropDown(anchor, -i, 0);
            if (dimBackground){
                setBackgroundAlpha(1f, alpha, 240);
            }
        }
    ...
    public TopRightMenu showAsDropDownM(View anchor,int diff,int diffY){
        if (mPopupWindow == null){
            getPopupWindow();
        }
        mPopupWindow.getContentView().measure(View.MeasureSpec.UNSPECIFIED,View.MeasureSpec.UNSPECIFIED);
        ...
        i=i-anchor.getWidth()/2-diff;
        if (!mPopupWindow.isShowing()) {
            mPopupWindow.showAsDropDown(anchor, -i, diffY);
            if (dimBackground){
                setBackgroundAlpha(1f, alpha, 240);
            }
        }
    ...
    public TopRightMenu showAsDropDownXy(View parent, int gravity, int x, int y){
        if (mPopupWindow == null){
            getPopupWindow();
        }
        if (!mPopupWindow.isShowing()) {
