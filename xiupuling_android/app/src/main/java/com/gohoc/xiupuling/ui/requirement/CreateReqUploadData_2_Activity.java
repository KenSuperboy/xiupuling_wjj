...
import com.gohoc.xiupuling.ui.LocalMusicListActivity;
...
/*
* 上传1-5张图片  ：图片（1-5张图片）  拍照   音频
* */
public class CreateReqUploadData_2_Activity extends BasicActivity implements OnUploadListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_create_req_upload_data_1);
        ...
        mVideoView.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                                             ...
                                             public void onPrepared(MediaPlayer mp) {
                                                 ...
                                                 //FixMe 获取屏幕的宽度
                                                 DisplayMetrics display = CreateReqUploadData_2_Activity.this.getResources().getDisplayMetrics();
    ...
    private void initBanner(final List<PicBean> picList) {
        ...
        for (int i=0;i<picList.size();i++){
            if(picList.get(i).isMusic()){
                playMediaStart(picList.get(i).getLocUrl());
                //picList.remove(i);
    ...
    private void initRv() {
        ...
        if (picList.size() == 0) {
            ...
            picList.add(picBean);
        } else if (picList.size() < 6 && mType == 2) {
            PicBean picBean = new PicBean();
            picBean.setStatus(100);
            picList.add(picBean);
        }
        LogUtil.d("设置前的长度："+picList.size());
        recyclerView.setLayoutManager(new GridLayoutManager(CreateReqUploadData_2_Activity.this, 2));
        recyclerView.addItemDecoration(new SimplePaddingDecoration(this));
        //设置adapter
        addPicAdater = new AddPicAdater_1(CreateReqUploadData_2_Activity.this, picList);

        addPicAdater.setMaxCount(6);
        /*if (mType == 2) {
            addPicAdater.setMaxCount(5);
        } else if (mType == 4) {
            addPicAdater.setMaxCount(2);
        } else {
            addPicAdater.setMaxCount(1);
        }*/
        ...
        recyclerView.addOnItemTouchListener(new OnRecyclerViewItemClickListener(recyclerView) {
            ...
            public void onLoncClick(RecyclerView.ViewHolder viewHolder) {
                ...
                if (viewHolder.getLayoutPosition() != picList.size() - 1) {
                    ...
                    VibratorUtil.Vibrate(CreateReqUploadData_2_Activity.this, 60);//ms
                }
            }
        ...
        addPicAdater.setOnPicChangeListion(
                new AddPicAdater_1.OnPicChangeListion() {
                    ...
                    public void OnDel(View v, int postion) {
                        ...
                        if (picList.get(picList.size() - 1).getType() == 1) {//type=1表示有数据
    ...
    protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (resultCode == ImagePicker.RESULT_CODE_ITEMS) {
                ...
                ThreadTask.getInstance().executorDBThread(new Runnable() {
                    ...
                    public void run() {
                        for (ImageItem imageItem : images) {
                            ...
                            imageItem.path = com.gohoc.xiupuling.utils.ImageUtils.compress(CreateReqUploadData_2_Activity.this, imageItem.path);
                            ...
                            if (addPicAdater.getItemCount() == addPicAdater.getMaxCount()) {
                                ...
                                picList.get(picList.size() - 1).setType(1).setStatus(0).setUpdatefileName(uploadKey).setLocUrl(imageItem.path).setPic(true);
                            ...
                            .setNormalDialogListenner(new NormalDialog.NormalDialogListenner() {
                                ...
                                public void onClick(NormalDialog.NormalType normalType) {
                                    if (normalType == NormalDialog.NormalType.OK) {
                                        isCancelled = false;
                                        PicBean picBean = picList.get(picList.size() - 1);
                                        picBean.setType(1)
            ...
            }else if(resultCode== BaseConstant.AUDIO_RESULT){
                //音乐文件
                LogUtil.d("音乐文件成功回调11111111");
                if (data != null && requestCode == BaseConstant.AUDIO_REQUEST) {
                    LogUtil.d("音乐文件成功回调222222222222");
                    LocalMusicBean localMusicBean= (LocalMusicBean) data.getSerializableExtra("audio");

                    if (addPicAdater.getItemCount() == addPicAdater.getMaxCount()) {
                        //最后一张
                        String uploadKey = Utils.getUploadKey(UpLoadConstant.REQUIREMENT, localMusicBean.getPath());
                        picList.get(picList.size() - 1).setType(1).setStatus(0).setUpdatefileName(uploadKey).setLocUrl(localMusicBean.getPath()).setMusic(true);
                        uploadFile(picList.get(picList.size() - 1), picList.size() - 1);
                    } else {
                        PicBean picBean = new PicBean();
                        picBean.setLocUrl(localMusicBean.getPath());
                        picBean.setType(1);
                        picBean.setMusic(true);
                        String uploadKey = Utils.getUploadKey(UpLoadConstant.REQUIREMENT, localMusicBean.getPath());
                        picBean.setUpdatefileName(uploadKey);
                        picList.add(picList.size() - 1, picBean);
                        uploadFile(picBean, picList.size() - 2);
    ...
    private void updatePicshow() {
        LogUtil.d("picList裁剪后1111的长度："+picList.size());
        showPicList.clear();
        showPicList.addAll(picList);
        LogUtil.d("showPicList裁剪后222的长度："+showPicList.size());
        if (picList.size() > 1 && picList.get(picList.size() - 1).getType() == 0)
            showPicList.remove(picList.size() - 1);
    ...
    private void uploadFile(final PicBean picBean, final int potions) {
        RxRetrofitClient.getInstance(CreateReqUploadData_2_Activity.this).getUploadToken(new Observer<VCodeBenan>() {
            @Override
            ...
            public void onError(Throwable e) {
                Utils.toast(CreateReqUploadData_2_Activity.this, "请检查网络是否正常");
                try {
                    throw e;
            ...
            public void onNext(VCodeBenan vCodeBenan) {
                if (vCodeBenan.getCode() == 1) {
                    ...
                    UploadManage.INSTANS.upload(picBean, potions, vCodeBenan, CreateReqUploadData_2_Activity.this);
                }
            }
        });
    }
    ...
    public void showDialog() {
        if(mSourceSubmitDialog==null){
            mSourceSubmitDialog =new SourceSubmitDialog(this, new SourceSubmitCallback() {
                ...
                @Override
                public void yinping() {
                    startActivityForResult(new Intent(mContext,LocalMusicListActivity.class),BaseConstant.AUDIO_REQUEST);
                    mSourceSubmitDialog.dismiss();
                }
        ...
        LogUtil.d(picList.size()+":判断列表："+picList);
        if((picList!=null&&picList.size()==0)||picList==null){
            LogUtil.d("首次进来：1111111111");
            mSourceSubmitDialog.setMyClick(true,false,true,false,true);
        }else {
            LogUtil.d("不是首次进来：22222222222");
            mSourceSubmitDialog.setMyClick(!CustomUtils.getPic_Five(picList),false,!CustomUtils.getMusic(picList),false,!CustomUtils.getPic_Five(picList));
        }
    ...
    private void showMenu(int type) {
        ...
        if (mType == 2) {
            ...
            if (picList != null) {
                ...
                for (PicBean b : picList) {
                    if (b.getLocUrl() != null && b.getLocUrl().length() > 0&&b.isPic()) {
                        count++;
                    }
                }
                limit = 5 - count;
        ...
        if (type == 0) {
            ...
            Intent intent = new Intent(CreateReqUploadData_2_Activity.this, ImageGridActivity.class);
            startActivityForResult(intent, IMAGE_PICKER);
        }
    }
    ...
    @OnClick({R.id.action_bar_left_title, R.id.action_bar_title, R.id.title_message_tv,  R.id.del_pic_iv, R.id.minus_iv, R.id.add_iv, R.id.play_iv, R.id.paus_iv, R.id.stop_iv, R.id.width_tv, R.id.height_tv, R.id.x_tv, R.id.y_tv, R.id.logout_button_ll})
    ...
    public void complete(String key, ResponseInfo info, JSONObject res, PicBean picBean) {
        ...
        if(picBean.isMusic()){
            LogUtil.d("本地音乐："+picBean.getLocUrl());
            playMediaStart(picBean.getLocUrl());
        }
