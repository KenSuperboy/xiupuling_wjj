...
* 定投发布----提交订单
* */
public class SubmitOrdersActivity extends BasicActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.req_time_dt_rl:
                startActivityForResult(new Intent(SubmitOrdersActivity.this, PushReqSelectTimeActivity.class).putExtra("selectedDays", selectedDays), TIME_REQUEST_RESULT);
                break;
            case R.id.button_ll:
                if (pushNormlBean.isFree()){
                    //先去统计信息
            ...
            case R.id.arr_right_iv2:
                startActivity(new Intent(SubmitOrdersActivity.this, PushReqChargesDtActivity.class)
            ...
            RxRetrofitClient.getInstance(this).rangeMarket(rq_id, work_id, range_type, startdate, enddate, addr_type
                    ...
                    , distance, new Observer<PushResultBean>() {
                        ...
                        public void onError(Throwable e) {
                            closeProgressDialog();
                            Utils.toast(SubmitOrdersActivity.this, "请检查网络是否正常");
                            try {
                                throw e;
                        ...
                        public void onNext(PushResultBean pushResultBean) {
                            ...
                            if (pushResultBean.getCode() == 1) {
                                if (pushNormlBean.isFree())//免费  免费的中间插了一个界面
                                    startActivity(new Intent(SubmitOrdersActivity.this, PushOrderPayReslutActivity.class).putExtra("pushResultBean", pushResultBean).putExtra("pushNormlBean", pushNormlBean));
                                else{
                                    startActivity(new Intent(SubmitOrdersActivity.this, PushOrderConfirmActivity.class).putExtra("pushResultBean", pushResultBean).putExtra("pushNormlBean", pushNormlBean));
                                }
                                    finish();
                            } else {
                                Utils.toast(SubmitOrdersActivity.this, pushResultBean.getMessage());
                            }
