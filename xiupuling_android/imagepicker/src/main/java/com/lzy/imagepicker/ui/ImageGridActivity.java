...
//        mImageGridAdapter = new ImageGridAdapter(this, null);
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.btn_ok) {
            Intent intent=new Intent(ImageGridActivity.this,ImageCropMultipleActivity.class);
            intent.putExtra("type",0);//正常情况下进来
...
//            intent.putExtra(ImagePicker.EXTRA_IMAGE_ITEMS, imagePicker.getCurrentImageFolderItems());
            ...
            if (imagePicker.isCrop()) {
                Log.d("TAG","图片选择---111111111--");
                Intent intent = new Intent(ImageGridActivity.this, ImageCropActivity.class);
                startActivityForResult(intent, ImagePicker.REQUEST_CODE_CROP);  //单选需要裁剪，进入裁剪界面
            } else {
                Log.d("TAG","图片选择----222222-");
                Intent intent = new Intent();
                intent.putExtra(ImagePicker.EXTRA_RESULT_ITEMS, imagePicker.getSelectedImages());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null && data.getExtras() != null) {
            ...
            if (resultCode == RESULT_OK && requestCode == ImagePicker.REQUEST_CODE_TAKE) {
                ...
                Log.d("TAG","333333333333333:"+imagePicker.getTakeImageFile());
                /**
                 * 2017-03-21 对机型做旋转处理
                 */
